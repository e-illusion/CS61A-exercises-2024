...
class Account:
    ...
    def deposit(self, amount):
        ...
        transaction = Transaction(self.index, balance, self.balance)
        self.index += 1
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            transaction = Transaction(self.index, self.balance, self.balance)
            self.index += 1
            self.transactions.append(transaction)
...
class Server:
    ...
    def send(self, email):
        ...
        client = self.clients.get(email.recipient_name)
        if client:
    ...
    if not coins:
        ...
        for coin in sorted(coins):
            if amount == coin:
                ...
                rest = remove_one(coins, coin)
                ret = make_change(amount-coin, rest)
                if ret:
                    return [coin] + ret
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        ret = make_change(coin, self.coins)
        if ret:
            for x in ret:
