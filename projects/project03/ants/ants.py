...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while True:
            if nearest_place.is_hive == True or cnt > self.upper_bound:
                return None
            elif nearest_place.bees and cnt >= self.lower_bound :
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if s:
            for bee in s:
                Ant.reduce_health(bee, min(amount, self.health))
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while tmp_place:
            ant = tmp_place.ant
            if ant:
                if ant.is_doubled == False:
                    ant.damage *= 2
                    ant.is_doubled = True
                if ant.is_container == True and ant.ant_contained and ant.ant_contained.is_doubled == False:
