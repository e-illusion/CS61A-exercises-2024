...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    list = [paragraph for paragraph in paragraphs if select(paragraph) == True]
    if k+1 <= len(list):
        return list[k]
    else:
        return ''
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def helper_select(object):
        helper_list = split(lower(remove_punctuation(object)),)
        for word in helper_list:
            for case in subject:
                if word == case:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        return 100.0
    cnt = sum(1 for i in range(0,min(len(typed_words),len(source_words))) if typed_words[i] == source_words[i])
    return cnt*100.0 /len(typed_words) if len(typed_words) > 0 else 0.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for element in word_list:
        diff = diff_function(typed_word, element, limit)
        if typed_word == element:
            return typed_word
        elif diff < min_diff:
            min_diff = diff
            ret = element
...
def report_progress(typed, source, user_id, upload):
    ...
    while i < len(typed) and typed[i] == source[i]:
        i += 1
    progress = i * 1.0 / len(source)
    rep = {'id': user_id, 'progress': progress}
    upload(rep)
    return progress
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in timestamps_per_player:
        ...
        for i in range(1, len(player)):
            player_time = player[i] - player[i-1]
            player_time_list.append(player_time)
        time_list.append(player_time_list)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    winner_list =[[] for i in player_indices] 
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < min_time:
                ...
                winner = j
        winner_list[winner].append(get_word(match, i))
    return winner_list
    # END PROBLEM 10
